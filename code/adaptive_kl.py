...
HYPERPARAMS = {
    ...
    #adaptive KL
    'beta': 1.0,  # 초기 KL Penalty 계수 (beta) adaptive KL 에서는 고정
    'kl_target': 0.05,  # 목표 KL 다이버전스 값 (d_targ)
}
...
def train_ppo(seed):
    print(f"--- Seed {seed}로 Adaptive KL Penalty 학습 시작 (Target KL={HYPERPARAMS['kl_target']}) ---")
    set_seed(seed)
    ...
    kl_target = HYPERPARAMS['kl_target']
    ...
    for update in range(1, num_updates + 1):
        ...
        for epoch in range(HYPERPARAMS['n_epochs']):
            ...
            for start in range(0, HYPERPARAMS['horizon'], HYPERPARAMS['minibatch_size']):
                ...
                pg_loss = -(surrogate_objective.mean() - current_beta * kl_div.mean())
        ...
        with torch.no_grad():
            ...
            if mean_kl > 1.5 * kl_target:
                beta *= 2.0
            elif mean_kl < 1.0 / 1.5 * kl_target:
                beta /= 2.0
        ...
        if len(episode_rewards) > 0:
            ...
            if update % 10 == 0:
                print(
                    f"[{update}/{num_updates}] Steps: {global_step} | Avg. Episode Reward (Last 10): {avg_ep_reward:.2f} | Current Beta: {beta:.4f} | Mean KL: {mean_kl:.4f}")
...
def plot_confidence_interval(all_results, window=10):
    ...
    ax.set_title(f'{HYPERPARAMS["env_id"]} PPO Adaptive KL Penalty (Target KL={HYPERPARAMS["kl_target"]})', fontsize=16)
    ...
    save_path = f'ppo_kl_adaptive_target_{HYPERPARAMS["kl_target"]}.png'
...
def main_experiment():
    ...
    print(f"\n==========================================")
    print(f"** PPO (Adaptive KL Penalty) 무작위 시드 실험 **")
