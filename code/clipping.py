...
def train_ppo(seed):
    print(f"--- Seed {seed}로 PPO-Clip 학습 시작 ---")
    set_seed(seed)
    ...
    for update in range(1, num_updates + 1):
        ...
        for _ in range(HYPERPARAMS['horizon']):
            ...
            if done:
                ...
                current_episode_reward = 0.0
                obs, _ = env.reset(seed=seed)  # Environment reset
        ...
        for _ in range(HYPERPARAMS['n_epochs']):
            ...
            for start in range(0, HYPERPARAMS['horizon'], HYPERPARAMS['minibatch_size']):
                ...
                # PPO-Clipping algorithm
                surrogate_objective1 = mb_adv * ratio
                clipped_ratio = torch.clamp(ratio, 1.0 - epsilon, 1.0 + epsilon)
                surrogate_objective2 = mb_adv * clipped_ratio

                pg_loss = -torch.min(surrogate_objective1, surrogate_objective2).mean()
...
def plot_confidence_interval(all_results, window=10):
    ...
    ax.set_title(
        f'{HYPERPARAMS["env_id"]} PPO Clipping (Epsilon={HYPERPARAMS["epsilon"]}) (Random Seeds)',
        fontsize=16)
    ...
    save_path = 'ppo_clip 0'+str(HYPERPARAMS["epsilon"]*10)+'.png'
...
def main_experiment():
    ...
    print(f"\n==========================================")
    print(f"** PPO-Clip (Epsilon={HYPERPARAMS['epsilon']}) 무작위 시드 실험 **")
